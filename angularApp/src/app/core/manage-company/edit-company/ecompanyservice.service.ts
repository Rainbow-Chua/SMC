...
export class ECompanyserviceService {
  ...
  deleteCompany(companyModuleModule: ECompanyModule): Observable<any> {
    this.API_URL = 'http://localhost:8090';
    // http://localhost:8090/views/index
    console.log(companyModuleModule);
    return this.http.post(this.API_URL + '/sba/api/deleteCompany', companyModuleModule , {
      observe: 'response',
      headers: this.headers,
      reportProgress: true
     });
  ...
  getCompany(param?: any): Observable<any> {
    this.API_URL = 'http://localhost:8090';

    return this.http.get(this.API_URL + '/sba/api/getCompany', { observe: 'response', params: param });
    }
