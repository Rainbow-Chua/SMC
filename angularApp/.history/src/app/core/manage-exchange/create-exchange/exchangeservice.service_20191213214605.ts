...
export class ExchangeserviceService {
  ...
  constructor(private http: HttpClient) {
   ...
   createCompany(exchangeModule: ExchangeModule): Observable<any> {
    this.API_URL = 'http://localhost:8090';
    // http://localhost:8090/views/index
    console.log(exchangeModule);
    return this.http.post(this.API_URL + '/sba/api/createExchange', exchangeModule , {
      observe: 'response',
  ...
  createExchange(param?: any): Observable<any> {
    this.API_URL = 'http://localhost:8090';
